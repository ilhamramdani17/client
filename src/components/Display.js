...
const Display = (props) => {
    ...
    function hapusSimpanan (id) {
        axios.post("http://localhost:8000/removeSave",{id: id,userid: props.user._id})
            .then(() => {
                props.setSetting(false)
            })
    }
    
    function hapusHide (id) {
        axios.post("http://localhost:8000/removeHide",{id: id,userid: props.user._id})
            .then(() => {
                props.setSetting(false)
            })
    }
    ...
    return (
        <div className="display">
            <div className="title">
            <strong>{props.datas.type + " "}{props.datas.desc ? props.datas.desc : ""}</strong>
            </div>
            <br/>
            <div className="container">
            {props.datas.type === "subscriber" || props.datas.type === "subscribing"
            ?
            ...
            <>
                {props.datas.data.map((post) => (
                    <div className="posts">
                        ...
                        <div className="overflow" onClick={() => handleDetail(post)}>
                            <p>{post.size}</p>
                            <p>{post.created_at}</p>
                            <p>{post.isi}</p>
                        </div>
                        ...
                        ? 
                            <>
                                {props.datas.desc !== "save"
                                ? <div className="savehide"><button onClick={() => hapusHide(post._id)}>Hapus Disembunyikan</button></div>
                                : <div className="savehide"><button onClick={() => hapusSimpanan(post._id)}>Hapus Simpanan</button></div>
                                }
                            </>
                        : 
                        <div className="info" onClick={() => handleDetail(post)}>
                        <div>
